...
action :install_server do
  ...
  if node[:db][:flavor] =~ /mariadb|tokudb/
     ...
     execute "#{node[:db_mysql][:tokudb][:base_dir]}/scripts/mysql_install_db --no-defaults --basedir=#{node[:db_mysql][:tokudb][:base_dir]} --datadir=#{node[:db_mysql][:datadir]} ; touch #{touchfile}" do
       creates touchfile
     end
  else
     touchfile = ::File.expand_path "~/.mysql_installed"
  ...
  case node[:db][:flavor]
   when "tokudb"
      template "/etc/mysql/debian-start" do
         ...
         variables(
            :basedir => node[:db_mysql][:tokudb][:base_dir]
         )
         cookbook "db_mysql"
      end
  ...
  if platform =~ /redhat|centos/
     ruby_block "Change init for #{platform}" do
        ...
        block do
           ...
           nc.search_file_replace_line(/^basedir/, "basedir=#{node[:db_mysql][:tokudb][:base_dir]}")
     ...
     ruby_block "Change init for ubuntu" do
        ...
        block do
           require 'chef/util/file_edit'
           nc = Chef::Util::FileEdit.new("/etc/init.d/mysql")
           nc.search_file_replace(/\/usr\/sbin\/mysqld/, "#{node[:db_mysql][:tokudb][:base_dir]}\/bin\/mysqld")
           nc.search_file_replace(/\/usr\/bin\/mysqld_safe/, "#{node[:db_mysql][:tokudb][:base_dir]}\/bin\/mysqld_safe")
           nc.search_file_replace(/\/usr\/bin\/mysqladmin/, "#{node[:db_mysql][:tokudb][:base_dir]}\/bin\/mysqladmin")
           nc.insert_line_after_match(/^set -u$/, "export MY_BASEDIR_VERSION=\"#{node[:db_mysql][:tokudb][:base_dir]}\"")
	   nc.insert_line_after_match(/^export MY_BASEDIR_VERSION/, "export PLUGIN_DIR=\"#{node[:db_mysql][:tokudb][:base_dir]}\/lib\/plugin\"")
           nc.write_file
        end
       end
   else
